...
| R012 | Never force-initialize or nuke native task history. | `assets/skills/oro/SKILL.md`, `assets/skills/oro/references/gotchas.md` | 6 | 4 | Dangerous-command hooks can catch known command strings. Promote to a runtime Bash hook denylist if the old command names remain reachable. |
| R013 | Do not use destructive commands without explicit confirmation. | `assets/skills/destructive-command-safety/SKILL.md` | 6 | 4 | `assets/hooks/destructive_command_guard.py` now blocks known destructive Bash payloads such as `rm -rf`, `git reset --hard`, `git checkout -f`, and `git branch -D`; force-push and intent-sensitive cases still require prompt discipline. |
...
| R013 | Level 4 | Extend Bash pre-tool destructive-command coverage to force-push and other intent-sensitive destructive operations. |
