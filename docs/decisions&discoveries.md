...
## 2026-07-12: Inline acceptance Cmd truncates at pipes — line-per-field is mandatory for piped commands
...
**Decision/Discovery:** (1) Any AC whose Cmd contains `|` MUST be stored in line-per-field format (parser splits on newlines only when the AC contains `\n`). (2) Bare `cmd | grep` pipelines in ACs also swallow the left command's exit code (sh -c has no pipefail) — use `cmd > log 2>&1 && grep -q ... log` instead. This is a latent trap for ALL epics, not just this design; task filed to harden the parser or validate ACs at creation time.
**Implications:** Cleanliness-roles design (docs/plans/2026-07-12-cleanliness-roles-design.md) mandates the line-per-field format explicitly. Until the parser is hardened, review any inline AC containing pipes.
