...
func TestStartCommandPreflightChecks(t *testing.T) {
	...
	withChdir(t, projectDir, func() {
		...
		waitForSocket(t, sockPath, socketPollTimeout)
...
func TestReconnectTmuxRecreatesUnhealthySession(t *testing.T) {
	t.Run("unhealthy session is killed and recreated", func(t *testing.T) {
		...
		// Stub new-session for recreation (managerless window)
		fake.output[key("tmux", "new-session", "-d", "-s", "oro", "-n", defaultTmuxWindowName)] = ""
