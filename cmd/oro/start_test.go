...
func TestRunFullStart_Detached(t *testing.T) {
	t.Run("skips attach and returns nil", func(t *testing.T) {
		...
		err = runFullStart(&buf, 2, "sonnet", "", spawner, tmuxRunner, func(int) error { return nil }, 2*time.Second, func(time.Duration) {}, 50*time.Millisecond, true, nil)
...
func TestStartSendsDirective(t *testing.T) {
	...
	var buf bytes.Buffer
	err = runFullStart(&buf, 2, "sonnet", "", spawner, tmuxRunner, func(int) error { return nil }, 2*time.Second, func(time.Duration) {}, 50*time.Millisecond, false, nil)
