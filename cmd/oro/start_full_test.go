...
func TestFullStart(t *testing.T) {
	t.Run("spawns daemon, waits for socket, creates tmux session, prints status", func(t *testing.T) {
		...
		err := runFullStart(&stdout, 3, "sonnet", "", spawner, fakeTmux, func(int) error { return nil }, 100*time.Millisecond, noopSleep, 50*time.Millisecond, false)
	...
	t.Run("returns error when daemon spawn fails", func(t *testing.T) {
		...
		var stdout bytes.Buffer
		err := runFullStart(&stdout, 2, "sonnet", "", spawner, newFakeCmd(), func(int) error { return nil }, 100*time.Millisecond, noopSleep, 50*time.Millisecond, false)
		if err == nil {
...
func TestCreateWithNudges(t *testing.T) {
	...
	t.Run("prints startup progress with checkmarks", func(t *testing.T) {
		...
		var stdout bytes.Buffer
		err := runFullStart(&stdout, 3, "sonnet", "", spawner, fakeTmux, func(int) error { return nil }, 100*time.Millisecond, noopSleep, 50*time.Millisecond, true)
		if err != nil {
