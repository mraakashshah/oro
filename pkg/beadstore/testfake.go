...
// SeedCards replaces the card contents with the supplied cards.
//
//oro:testonly
func (s *FakeStore) SeedCards(cs []cards.Card) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// Cards() returns a fakeCardsReadTx backed by the cards seeded via SeedCards.
//
//oro:testonly
func (s *FakeStore) WithReadTx(_ context.Context, fn func(tx ReadTx) error) error {
	...
	s.mu.RUnlock()
	return fn(&fakeReadTx{s: s, cardsTx: &fakeCardsReadTx{cs: snapshot}})
}
...
// Cards implements ReadTx. Returns a fakeCardsReadTx backed by SeedCards data.
func (r *fakeReadTx) Cards() cards.ReadTx { return r.cardsTx }

// fakeCardsReadTx implements cards.ReadTx over an in-memory card slice.
//
//oro:testonly
type fakeCardsReadTx struct{ cs []cards.Card }

// Show implements cards.ReadTx.
func (r *fakeCardsReadTx) Show(_ context.Context, id string) (*cards.Card, error) {
	for _, c := range r.cs {
		if c.ID == id {
			clone := c
			return &clone, nil
...
func (r *fakeCardsReadTx) List(_ context.Context, q cards.ListQuery) ([]cards.Card, error) {
	var out []cards.Card
	for _, c := range r.cs {
...
// Relevant implements cards.ReadTx using the exported scoring functions.
func (r *fakeCardsReadTx) Relevant(_ context.Context, q cards.RelevanceQuery) (cards.RelevantCards, error) {
	now := time.Now()
	type scored struct {
		c     cards.Card
		score float64
	...
	for _, c := range r.cs {
		...
		candidates = append(candidates, scored{c: c, score: eff})
	}
	sort.Slice(candidates, func(i, j int) bool { return candidates[i].score > candidates[j].score })

	deck := make([]cards.CardSummary, 0, len(candidates))
	for _, sc := range candidates {
		deck = append(deck, fakeCardSummary(sc.c))
	}
	...
	if q.MaxTokens > 0 {
		budget := q.MaxTokens
		for _, sc := range candidates {
			tokens := (len(sc.c.BodyFull) + 3) / 4
			if tokens > budget {
				break
			}
			budget -= tokens
			inlined = append(inlined, fakeCardSummary(sc.c))
		}
	}
...
func fakeCardSummary(c cards.Card) cards.CardSummary {
