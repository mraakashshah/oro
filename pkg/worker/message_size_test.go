...
func TestLoadThresholds(t *testing.T) {
	t.Run("loads from file", func(t *testing.T) {
		dir := t.TempDir()
		data := `{"opus": 40, "sonnet": 40, "haiku": 40}`
		...
		if th.For("opus") != 40 {
			t.Errorf("opus: got %d, want 40", th.For("opus"))
		}
		if th.For("sonnet") != 40 {
			t.Errorf("sonnet: got %d, want 40", th.For("sonnet"))
		}
		if th.For("haiku") != 40 {
	...
	t.Run("falls back to default when file missing", func(t *testing.T) {
		...
		if th.For("opus") != 40 {
			t.Errorf("opus default: got %d, want 40", th.For("opus"))
		}
	})
	...
	t.Run("falls back to default for unknown model", func(t *testing.T) {
		...
		if th.For("unknown-model") != 40 {
			t.Errorf("unknown model: got %d, want 40", th.For("unknown-model"))
		}
	})
}
