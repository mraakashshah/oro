...
func buildDecomposePrompt(opts DecomposeOpts) string {
	...
	fmt.Fprintf(&b, "   a. `oro bead create --title=\"...\" --type=task --acceptance=\"...\" --estimate=<min>`\n")
	fmt.Fprintf(&b, "   b. `oro bead update <child-id> --parent=%s`  (sets hierarchy only, no dep)\n", opts.BeadID)
	fmt.Fprintf(&b, "   c. `oro bead dep add %s <child-id>`  (epic depends on child — correct direction)\n", opts.BeadID)
