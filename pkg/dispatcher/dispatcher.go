...
func (d *Dispatcher) admitHello(conn net.Conn, msg protocol.Message) (string, bool) {
	...
	if err := json.NewEncoder(conn).Encode(protocol.Message{
		...
		Protocol: d.helloMetadata(msg.Protocol.WorkerID, msg.Protocol.WorkerGeneration),
		HelloACK: &ack,
	}); err != nil {
		return "", false
	}
...
func (d *Dispatcher) handleHello(_ net.Conn, msg protocol.Message) (protocol.HelloACK, error) {
	...
	expected := d.helloIdentity(msg.Protocol.WorkerID, msg.Protocol.WorkerGeneration)
	// Restart generation is assigned in HELLO_ACK. A worker cannot know the
	// generation of a dispatcher that has not acknowledged it yet.
	expected.RestartGeneration = 0
	if err := msg.Validate(d.helloSupportedRange, expected); err != nil {
...
func (d *Dispatcher) helloMetadata(workerID string, workerGeneration uint64) protocol.Metadata {
	return protocol.Metadata{
		ProtocolRange:     d.helloSupportedRange,
		ProjectID:         d.helloProjectID,
		WorkerID:          workerID,
		WorkerGeneration:  workerGeneration,
		RestartGeneration: d.helloRestartGeneration,
