...
// QGFeedbackRef identifies immutable QG feedback content by its SHA-256
// digest. Named distinctly from ArtifactRef in review_checkpoint_store.go,
// which is a retention-pruning path reference and an unrelated concept.
type QGFeedbackRef struct{ SHA256 string }
...
func (d *Dispatcher) rememberQGRetryContext(workerID string, rec QGFailureRecord, attempt int) {
	...
	retry := QGRetryContext{OccurrenceID: rec.ID, HeadSHA: strings.TrimSpace(string(head)), Attempt: attempt, FeedbackRef: &QGFeedbackRef{SHA256: rec.OutputHash}}
	d.mu.Lock()
