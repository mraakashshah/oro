...
ON CONFLICT(bead_id) DO NOTHING`, beadID, token, beadID)
	...
	if err != nil {
		return nil, fmt.Errorf("count assignment side-effect admission for %s: %w", beadID, err)
	}
	if rows != 1 {
...
func (d *Dispatcher) releaseAssignmentSideEffectAdmission(ctx context.Context, admission *assignmentSideEffectAdmission) {
	...
	defer cancel()
	if _, err := d.db.ExecContext(releaseCtx, `
DELETE FROM assignment_side_effect_admissions WHERE bead_id=? AND owner_token=?`, admission.beadID, admission.token); err != nil {
