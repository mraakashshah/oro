...
// TestRestartWorkerResetsBead verifies that applyRestartWorker:
//  1. Calls BeadSource.Update(beadID, "open") to reset bead status.
//  2. Calls clearBeadTracking to remove all tracking-map entries for the bead.
//  3. Keeps targetWorkers unchanged (unlike kill-worker which decrements it).
func TestRestartWorkerResetsBead(t *testing.T) {
	const workerID = "w-restart-test"
	const beadID = "oro-restart1"

	t.Run("managed worker: bead reset to open, tracking cleared, targetWorkers unchanged", func(t *testing.T) {
		d, beadSrc, _, _, _, _ := newTestDispatcher(t)
		...
		d.workers[workerID] = &trackedWorker{
			...
			state:    protocol.WorkerBusy,
			beadID:   beadID,
			worktree: "/tmp/worktrees/oro-restart1",
			managed:  true,
			encoder:  json.NewEncoder(conn),
		}
		// Seed tracking maps so we can verify clearBeadTracking.
		...
		if err != nil {
			t.Fatalf("applyRestartWorker returned error: %v", err)
		}

		// 1. BeadSource.Update must reset bead to "open".
		beadSrc.mu.Lock()
		status := beadSrc.updated[beadID]
		beadSrc.mu.Unlock()
		if status != "open" {
		...
		// 2. Tracking maps must be cleared.
		d.mu.Lock()
		attempts := d.attemptCounts[beadID]
		handoffs := d.handoffCounts[beadID]
		rejections := d.rejectionCounts[beadID]
		...
		// 3. targetWorkers must remain unchanged (unlike kill-worker).
		d.mu.Lock()
		target := d.targetWorkers
		d.mu.Unlock()
		if target != 2 {
			t.Errorf("targetWorkers = %d, want 2 (should remain unchanged after restart)", target)
		}
	})
}
