...
func TestKeyWNoopWhenWorkUnavailable(t *testing.T) {
	m := setupModel(t)
	m.workAvail = false

	_, cmd := m.Update(tea.KeyPressMsg{Code: 'w', Text: "w"})
	if cmd != nil {
		t.Fatal("expected nil cmd from w when workAvail=false")
	}
}

func TestKeyWLaunchesCmd(t *testing.T) {
	m := setupModel(t)
	m.workAvail = true
	m.inTmux = false // use ExecProcess path, not tmux

	_, cmd := m.Update(tea.KeyPressMsg{Code: 'w', Text: "w"})
	if cmd == nil {
		t.Fatal("expected non-nil cmd from w when workAvail=true and issue selected")
	}
}
...
func TestKeyWShiftNoopWhenNoActiveWorker(t *testing.T) {
	m := setupModel(t)
	// no activeWorkers set

	_, cmd := m.Update(tea.KeyPressMsg{Code: 'W', Text: "W"})
	if cmd != nil {
		t.Fatal("expected nil cmd from W when no active worker")
	}
}
