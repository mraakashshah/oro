...
- **One-off priority work**: for P0 beads that need immediate attention without changing the target count, use `oro worker launch --bead <bead-id>` to reserve targeted capacity through the dispatcher.
...
- **Update cadence**: Update the handoff file every 30 minutes or when swarm state changes significantly (e.g., all workers complete their beads, a critical escalation occurs).
- **File path**: Keep handoff updates in `docs/handoffs/current.yaml` for persistence across manager sessions.
- **Handoff content**: Include current backlog status (ready/blocked/in-progress counts), active worker beads, dispatcher state, any escalations in progress, and recommendations for the next session.
